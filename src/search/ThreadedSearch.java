...
public class ThreadedSearch<T> implements Searcher<T>, Runnable {
    ...
    public boolean search(T target, List<T> list) throws InterruptedException {
       try {
         ...
         int searchIncrement = list.size()/numThreads;

         // Creates `numThreads` of Threads with their section to search on the list
         Thread threads[] = new Thread[numThreads];
         for(int i=0; i<numThreads; i++) {
           threads[i] = new Thread(new ThreadedSearch<T>(target, list, i*searchIncrement, (i+1)*searchIncrement, answer));
           threads[i].start();
          }
    ...
    public void run() {
        for(int j = begin; j < end; j++) {
          ...
          // Breaks for loop if a thread has found the answer
